    ...
    T: Write + Seek,
{
    ...
    pub fn write_qword_seek(&mut self, seek_from: SeekFrom, value: u64) -> io::Result<()> {
    ...
    fn seek(&mut self, seek_from: SeekFrom) -> io::Result<u64> {
        self.current_offset = self.writer.seek(seek_from)?;
...
mod test {
    ...
    fn errors_when_out_of_space() {
        ...
        let mut assembler = Emitter::new(&mut writer).unwrap();
        assert_eq!(
    ...
    fn writes_byte() -> io::Result<()> {
        let mut writer = create_writer(1);
        let mut assembler = Emitter::new(&mut writer)?;
